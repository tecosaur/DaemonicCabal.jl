...
# reattachment — the conductor tears it down via drop_session when its label
# expires (matching JULIA_DAEMON_LABEL_TTL).
...
function get_or_create_session(label::String, client_stdout::StreamIO,
    ...
    @lock STATE.lock begin
        ...
        if isnothing(session)
            ...
            outs, errs, sigs = attach ?
                (StreamIO[client_stdout], StreamIO[client_stderr], StreamIO[signals]) :
                (StreamIO[], StreamIO[], StreamIO[])
            session = SyncSession(pipe.out, pipe.in,
                                  BroadcastWriter(outs, history), BroadcastWriter(errs, history),
                                  sigs, history, attach ? 1 : 0, Ref{REPL.LineEditREPL}())
