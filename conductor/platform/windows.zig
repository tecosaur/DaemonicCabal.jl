...
fn connectAfd(fd: HANDLE, addr: *const posix.sockaddr, len: posix.socklen_t) !void {
    switch (addr.family) {
    ...
    _ = try syncAfdControl(fd, win32.IOCTL.AFD.CONNECT, @as([]const u8, @ptrCast(&storage))[0 .. @offsetOf(ConnectStorage, "addr") + len], &.{});
}
...
/// Open the client end of a pipe, waiting briefly for a listening instance
/// (a server re-creates its instance right after each accept).
fn connectPipe(name: []const u8) !HANDLE {
    var wide: [max_local_addr + 1]u16 = undefined;
    const path = try pipeWin32Name(&wide, name);
    ...
    while (attempts < 200) : (attempts += 1) {
        ...
        switch (ntdll.NtCreateFile(
            ...
            // Between instances, or not yet created: retry for ~10s.
            .PIPE_BUSY, .OBJECT_NAME_NOT_FOUND, .INSTANCE_NOT_AVAILABLE, .PIPE_NOT_AVAILABLE, .PIPE_CLOSING => {},
            else => |status| return win32.unexpectedStatus(status),
        }
        Sleep(50);
    }
...
pub fn rawConnectLocal(path: []const u8) ?HANDLE {
    return connectPipe(path) catch null;
}
